    ...
    for (String stationId in stationIds) {
      try {
        final response = await http
            .get(Uri.parse('http://localhost:8000/stations/$stationId'));
        if (response.statusCode == 200) {
          final station = json.decode(response.body);
          setState(() {
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          leading: const Icon(Icons.local_parking,
                              color: Colors.white),
                          title: Text(
                            '${stationMap[booking['stationId']] != null ? stationMap[booking['stationId']]['name'] : 'Unknown Station'} - Slot ${booking['slotId'] ?? 'N/A'}',
